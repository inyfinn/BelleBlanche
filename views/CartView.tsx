        ...
        <div className="flex items-center py-5">
            ...
            <div className="ml-4 flex-1 flex flex-col justify-between">
                <div>
                    ...
                    <p className="text-sm text-secondary font-bold mt-1">{parseFloat(item.price).toFixed(2)} zł</p>
                </div>
                <div className="flex items-center mt-3">
                    <div className="flex items-center bg-accent rounded-full">
                        <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="p-2 text-dark"><MinusIcon className="w-4 h-4" /></button>
                        <span className="px-3 text-sm font-bold">{item.quantity}</span>
                        <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="p-2 text-dark"><PlusIcon className="w-4 h-4" /></button>
                    </div>
                </div>
            </div>
             <div className="pl-4">
                <button onClick={() => removeFromCart(item.id)} className="text-gray-400 hover:text-red-500"><XMarkIcon className="w-6 h-6"/></button>
            </div>
        </div>
    )
}
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mt-8 bg-white p-6 rounded-2xl shadow-sm">
        ...
        <button 
            ...
            className="mt-6 w-full bg-secondary text-white py-4 rounded-xl font-bold hover:bg-opacity-90 transition-colors text-lg"
        >
            Do kasy
        </button>
      </div>
